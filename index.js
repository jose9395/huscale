...
app.post("/process-images", upload.array("pictures", 4), async (req, res) => {
  ...
  try {
    const imageParts = await Promise.all(
      files.map((file) => fileToGenerativePart(file.buffer, file.mimetype))
    );
